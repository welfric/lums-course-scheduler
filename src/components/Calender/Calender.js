      ...
      <div className="InnerContainer">
        <div className="Time">
          {timeSlots.map((timeSlot) => {
            const timeID =
              timeSlot > 12 ? "PM" + (timeSlot - 12) : "AM" + timeSlot;
            const printTime =
              timeSlot > 12 ? timeSlot - 12 + ":00 PM" : timeSlot + ":00 AM";
        ...
        <div className="days">
          {days.map((day) => (
            <div key={day} style={{ height: "100%", width: "calc(1100px/8)" }}>
          ...
          <div className="Classes">
            {SelectedCourses.map((course) => {
              console.log("In div classes", SelectedCourses);
              const height =
                (parseInt(parseTimeLength(course.Day_and_Time)) / (14 * 60)) *
                calenderHeight;
              const width = 1100 / 8;
              ...
              for (let day = 0; day < classDays.length; day++) {
                ...
                courseCopyFinal.marginLeft =
                  days.findIndex(
                    (currentDay) => currentDay === classDays[day]
                  ) *
                  (1100 / 8);
